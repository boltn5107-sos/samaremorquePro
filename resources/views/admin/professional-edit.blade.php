...
@section('content')
    <section class="relative overflow-hidden reveal">
        <div class="max-w-3xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
            <a href="{{ route('admin.professionnels.show', $professional) }}" class="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-900 mb-5">
            ...
            <div class="flex items-center gap-3 mb-6">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-slate-900">Editer {{ $professional->full_name }}</h1>
            ...
            @if($errors->any())
                <div class="bg-red-50 border border-red-200 rounded-2xl p-4 mb-5">
                    <div class="flex items-start gap-2">
                        <x-icon name="alert-triangle" class="w-5 h-5 text-red-600 flex-shrink-0 mt-0.5" />
                        <div>
            ...
            <form method="POST" action="{{ route('admin.professionnels.update', $professional) }}" class="card p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea id="bio" name="bio" rows="3"
                              class="w-full rounded-xl border-slate-300 text-sm focus:ring-orange-500 focus:border-orange-500">{{ old('bio', $professional->bio) }}</textarea>
                </div>
