        ...
        <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-slate-900">Detail intervention #{{ $intervention->id }}</h1>
                    ...
                    <div class="card p-6">
                        ...
                        <dl class="space-y-4 text-sm">
                            <div>
                                ...
                                <dd class="font-medium text-slate-900">{{ $intervention->client_name }}</dd>
                                <dd class="mt-1.5">
                                    @if($intervention->isGuest())
                                        <span class="badge bg-orange-100 text-orange-700">Demande sans compte ({{ $intervention->tracking_code }})</span>
                                    @else
                                ...
                                <dd class="mt-2 flex flex-wrap gap-1.5">
                                    <a href="tel:{{ $intervention->client_phone }}" class="btn-secondary px-3 py-1 text-xs {{ $intervention->client_phone ? '' : 'pointer-events-none opacity-50' }}">
                                        <x-icon name="phone" class="w-3 h-3" /> {{ $intervention->client_phone ?? 'Numero non renseigne' }}
                                    </a>
                                    <a href="{{ $clWa }}" target="_blank" rel="noopener" class="px-3 py-1 text-xs font-semibold rounded-lg text-white bg-emerald-600 hover:bg-emerald-700 inline-flex items-center gap-1.5 {{ $intervention->client_phone ? '' : 'pointer-events-none opacity-50' }}">
                            ...
                            @if($intervention->photo)
                                <div>
                                    ...
                                    <a href="{{ asset('storage/' . $intervention->photo) }}" target="_blank" rel="noopener">
                                        <img src="{{ asset('storage/' . $intervention->photo) }}" alt="Photo de la panne" class="mt-1.5 w-full max-w-xs rounded-xl border border-slate-200">
                                    </a>
                                </div>
                            @endif
                        </dl>
