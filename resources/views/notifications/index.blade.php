        ...
        <div class="max-w-3xl mx-auto py-10 px-4 sm:px-6 lg:px-8 relative">
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-8">
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-500">{{ $notifications->count() }} notification{{ $notifications->count() > 1 ? 's' : '' }}</p>
                </div>
            ...
            <div class="bg-white shadow-card rounded-2xl border border-slate-100 overflow-hidden">
                <ul class="divide-y divide-slate-100">
                    @forelse($notifications as $notification)
                        <li class="group transition-colors duration-300 {{ is_null($notification->read_at) ? 'bg-gradient-to-r from-orange-50/80 via-white to-white' : 'bg-white hover:bg-slate-50/60' }}">
                                ...
                                <div class="flex items-start justify-between gap-3">
                                    <div class="flex items-start gap-3 min-w-0 flex-1">
                                        ...
                                        <div class="min-w-0 flex-1">
                                            <p class="text-sm font-semibold {{ is_null($notification->read_at) ? 'text-orange-600' : 'text-slate-700' }}">{{ $notification->data['title'] ?? $notification->type }}</p>
                                            <p class="mt-0.5 text-sm text-slate-500 line-clamp-2">{{ $notification->data['body'] ?? '' }}</p>
                                        </div>
                                    </div>
