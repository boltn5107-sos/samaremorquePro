        ...
        <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="card p-6">
                        <h2 class="text-lg font-semibold text-slate-900 mb-4 flex items-center gap-2">
                            <x-icon name="car" class="w-5 h-5 text-orange-500" />
                            Informations de la demande
                        </h2>
                        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
                            <div>
                                <dt class="text-slate-500 flex items-center gap-1.5"><x-icon name="zap" class="w-4 h-4" /> Service</dt>
                                <dd class="font-medium text-slate-900 mt-0.5">{{ ucfirst($intervention->service_type) }}</dd>
                            </div>
                            <div>
                                <dt class="text-slate-500 flex items-center gap-1.5"><x-icon name="car" class="w-4 h-4" /> Vehicule</dt>
                                <dd class="font-medium text-slate-900 mt-0.5">{{ ucfirst($intervention->vehicle_type) }}</dd>
                            </div>
                            <div>
                                <dt class="text-slate-500 flex items-center gap-1.5"><x-icon name="map-pin" class="w-4 h-4" /> Destination</dt>
                                <dd class="font-medium text-slate-900 mt-0.5">{{ $intervention->destination }}</dd>
                            </div>
                            @if($intervention->destination_lat && $intervention->destination_lng)
                                <div>
                                    <dt class="text-slate-500 flex items-center gap-1.5"><x-icon name="location" class="w-4 h-4" /> Coordonnees destination</dt>
                                    <dd class="font-medium text-slate-900 mt-0.5">{{ $intervention->destination_lat }}, {{ $intervention->destination_lng }}</dd>
                                </div>
                            @endif
                            @if($intervention->distance_km !== null)
                                <div>
                                    <dt class="text-slate-500 flex items-center gap-1.5"><x-icon name="map" class="w-4 h-4" /> Distance</dt>
                                    <dd class="font-medium text-slate-900 mt-0.5">{{ number_format((float) $intervention->distance_km, 1, ',', ' ') }} km</dd>
                                </div>
                            @endif
                            @if($intervention->estimated_duration_minutes)
                                <div>
                                    <dt class="text-slate-500 flex items-center gap-1.5"><x-icon name="clock" class="w-4 h-4" /> Duree estimee</dt>
                                    <dd class="font-medium text-slate-900 mt-0.5">{{ $intervention->estimated_duration_minutes }} min</dd>
                                </div>
                            @endif
                            @if($intervention->description)
                                <div class="sm:col-span-2">
                                    <dt class="text-slate-500 flex items-center gap-1.5"><x-icon name="alert-triangle" class="w-4 h-4" /> Description</dt>
                                    <dd class="font-medium text-slate-700 mt-0.5">{{ $intervention->description }}</dd>
                                </div>
                            @endif
                            @if($intervention->photo)
                                <div class="sm:col-span-2">
                                    ...
                                    <a href="{{ asset('storage/' . $intervention->photo) }}" target="_blank" rel="noopener" class="inline-block mt-2">
                                        <img src="{{ asset('storage/' . $intervention->photo) }}" alt="Photo de la panne" class="w-full max-w-xs rounded-lg border border-slate-200 shadow-sm hover:opacity-90 transition-opacity">
                                    </a>
                                </div>
                            @endif
                        </dl>
                        ...
                        <div class="card p-6">
                            ...
                            <div class="space-y-3">
                                @foreach($intervention->nextStatuses() as $next)
                                    <form method="POST" action="{{ route('remorqueur.intervention.status', $intervention) }}"
                ...
                <div class="space-y-6">
                    <div class="card p-6">
                        ...
                        <div class="flex items-center gap-3 mb-4">
                            @if($intervention->client_photo)
                                <img src="{{ asset('storage/' . $intervention->client_photo) }}" alt="{{ $intervention->client_name }}" class="w-12 h-12 rounded-full object-cover bg-slate-100">
                            @else
                            ...
                            @endif
                            <div class="min-w-0">
                                <p class="font-semibold text-slate-900 truncate">{{ $intervention->client_name }}</p>
                                <p class="flex items-center gap-1 text-sm text-slate-500">
                                    <x-icon name="phone" class="w-3.5 h-3.5" />
                                    {{ $intervention->client_phone ?? 'Numero non renseigne' }}
                                </p>
                                @if($intervention->isGuest())
                                    <p class="text-xs text-slate-400">Demande sans compte ({{ $intervention->tracking_code }})</p>
                                @else
                        ...
                        <div class="flex gap-2">
                            <a href="tel:{{ $intervention->client_phone }}" class="btn-secondary flex-1 text-sm py-2 {{ $intervention->client_phone ? '' : 'pointer-events-none opacity-50' }}">
                                <x-icon name="phone" class="w-4 h-4" />
                                Appeler
                            </a>
                            <a href="{{ $clientWhatsapp }}" target="_blank" rel="noopener" class="px-4 py-2 text-sm font-semibold rounded-xl text-white bg-emerald-600 hover:bg-emerald-700 inline-flex items-center justify-center gap-2 flex-1 {{ $intervention->client_phone ? '' : 'pointer-events-none opacity-50' }}">
                    ...
                    <div class="card p-6">
                        <h2 class="text-lg font-semibold text-slate-900 mb-4 flex items-center gap-2">
                            <x-icon name="shield" class="w-5 h-5 text-orange-500" />
                            Professionnel
                        </h2>
                        <div class="flex items-center gap-3">
                            @if(Auth::user()->photo)
                                <img src="{{ asset('storage/' . Auth::user()->photo) }}" alt="Votre profil" class="w-12 h-12 rounded-full object-cover bg-slate-100">
                            @else
                                @php $proInitials = collect(preg_split('/\s+/', trim(Auth::user()->full_name ?: Auth::user()->name ?? 'Professionnel')))->map(fn ($w) => strtoupper(mb_substr($w, 0, 1)))->take(2)->implode(''); @endphp
                                <div class="w-12 h-12 rounded-full flex items-center justify-center bg-slate-900 text-white font-semibold">{{ $proInitials ?: '?' }}</div>
                            @endif
                            <div class="min-w-0">
                                <p class="font-semibold text-slate-900 truncate">{{ Auth::user()->full_name ?: Auth::user()->name }}</p>
                                <p class="flex items-center gap-1 text-sm text-slate-500">
                                    <x-icon name="phone" class="w-3.5 h-3.5" />
                                    {{ Auth::user()->phone ?? 'Numero non renseigne' }}
                                </p>
