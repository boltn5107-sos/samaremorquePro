        ...
        <div class="max-w-3xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between mb-6">
                ...
                @if(!$interventions->isEmpty())
                    <span class="badge bg-orange-100 text-orange-700">{{ $interventions->count() }} en attente</span>
                @endif
            </div>
                        ...
                        <div class="card p-5">
                            <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                                <div class="flex gap-4 min-w-0">
                                    <div class="flex-shrink-0">
                                        @if($intervention->client_photo)
                                            <img src="{{ asset('storage/' . $intervention->client_photo) }}" alt="{{ $intervention->client_name }}" class="w-12 h-12 rounded-full object-cover bg-slate-100 ring-2 ring-white shadow-sm">
                                        @else
                                            ...
                                            <div class="w-12 h-12 rounded-full flex items-center justify-center bg-orange-100 text-orange-600 font-semibold ring-2 ring-white shadow-sm">{{ $clientInitials ?: '?' }}</div>
                                        @endif
                                    </div>
                                    ...
                                    <div class="space-y-1.5 min-w-0">
                                        ...
                                        <p class="flex items-center gap-1.5 text-sm text-slate-600">
                                            <x-icon name="user" class="w-4 h-4 text-slate-400 flex-shrink-0" />
                                            <span class="truncate">{{ $intervention->client_name }}</span>
                                        </p>

                                        <p class="flex items-center gap-1.5 text-sm text-slate-600">
                                            <x-icon name="phone" class="w-4 h-4 text-slate-400 flex-shrink-0" />
                                        ...
                                        <div class="flex gap-2 pt-1.5">
                                            <a href="tel:{{ $intervention->client_phone }}" class="btn-secondary px-3 py-1.5 text-xs {{ $intervention->client_phone ? '' : 'pointer-events-none opacity-50' }}">
                                                <x-icon name="phone" class="w-3.5 h-3.5" />
                                                Appeler
                                            </a>
                                            <a href="{{ $cWhatsapp }}" target="_blank" rel="noopener" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg text-white bg-emerald-600 hover:bg-emerald-700 {{ $intervention->client_phone ? '' : 'pointer-events-none opacity-50' }}">
                                        ...
                                        <p class="flex items-center gap-1.5 text-xs text-slate-400">
                                            ...
                                            Demandee {{ $intervention->created_at->diffForHumans() }}
                                        </p>

                                        @if($intervention->photo)
                                            <a href="{{ asset('storage/' . $intervention->photo) }}" target="_blank" rel="noopener" class="block mt-2">
                                                <img src="{{ asset('storage/' . $intervention->photo) }}" alt="Photo de la panne" class="w-28 h-28 object-cover rounded-lg border border-slate-200 shadow-sm hover:opacity-90 transition-opacity">
                                            </a>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="flex gap-2 sm:flex-col sm:w-40 flex-shrink-0">
                                    <form method="POST" action="{{ route('remorqueur.intervention.accept', $intervention) }}" class="flex-1"
